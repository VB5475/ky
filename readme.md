...
Returns a [`Response` object](https://developer.mozilla.org/en-US/docs/Web/API/Response) with [`Body` methods](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch#body) added for convenience. So you can, for example, call `ky.get(input).json()` directly without having to await the `Response` first. When called like that, an appropriate `Accept` header will be set depending on the body method used. Unlike the `Body` methods of `window.Fetch`, these will throw an `HTTPError` if the response status is not in the range of `200...299`. Also, `.json()` will return the value of `noContentValue` (defaults to empty string) if body is empty or the response status is `204` instead of throwing a parse error due to an empty body.
...
Value returned by `.json()` when the response is empty (including `204 No Content`). Set this to `null` or any other value to avoid optional chaining pitfalls when handling empty responses.
